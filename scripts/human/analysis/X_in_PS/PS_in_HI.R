...
HI_column <-  'is_HI'
phase_sep_column <- "dr_all_phase_sep"
abundance_groups <-  5
disorder_groups <-  5
alpha = 0.01
...
filter_PS <- function(bin, column_name) {
  PS_bin <- filter(bin, !!sym(column_name) == 1)
  return(PS_bin)
}

aa <- binned_genes$`3,3` %>%
  filter(dr_all_phase_sep == 1)
PS_bins <- lapply(binned_genes, filter_PS, phase_sep_column)

### 4) Calc enrichment of phase sep proteins in HI bins

enrichment_list <- lapply(PS_bins, summarize_enrichment, enrichment_column = HI_column)

### 5) calc enrichment of proteome as a whole

all_PS <- master %>%
  filter(!!(sym(phase_sep_column)) == 1)
proteome_enrichment <- summarize_enrichment(all_PS, enrichment_column = HI_column)
