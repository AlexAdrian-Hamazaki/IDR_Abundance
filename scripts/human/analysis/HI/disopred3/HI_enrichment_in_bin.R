#Purpose:
#Identify if proteins that are HI are enriched for high disorder high abundance
# Aka, if proteins that are HI are enriched in a given bin.
#In human
...
### 3) Get total amount of HI genes in each bin, and how many are in total
...
pheatmap(mat = p_value_matrix,
         main = paste0("Abundance:",abundance_groups, " x ", "Disorder:",disorder_groups, " Pvalue heatmap for enrichment of HI"),
         breaks = c(0, alpha, 1),
         cluster_cols = FALSE,
         cluster_rows = FALSE,
         color = c('red4', 'grey'),
